...
export class StateManager<T extends object> {
  ...
  /**
   * Атомарно вычисляет следующее состояние из текущего снимка.
   *
   * Producer получает замороженный shallow-снимок и должен вернуть полный следующий объект.
   * Подписчики уведомляются ровно один раз после замены состояния.
   */
  update(producer: (state: Readonly<T>) => T): void {
    this.state = { ...producer(this.get()) };
    this.notify();
  }
