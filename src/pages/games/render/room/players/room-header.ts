...
export function renderRoomRankedToggle(room: GameRoom, canManageRanked: boolean): string {
  const lockedHint = gameT("room.rankedLocked");
  ...
  return `
    <section class="games-room-detail-card games-room-detail-card--ranked" aria-label="${escapeHtml(gameT("room.gameTypeLabel"))}">
      <div class="games-room-detail-card__content">
        <span class="games-room-detail-card__label">${escapeHtml(gameT("room.gameTypeLabel"))}</span>
      </div>
      <div class="games-tooltip-anchor games-tooltip-anchor--ranked-toggle${tooltipClass}">
        <fieldset class="games-ready-segmented games-ready-segmented--compact games-rating-segmented games-room-ranked-segmented${lockedClass}" aria-label="${escapeHtml(gameT("room.gameTypeLabel"))}">
          <label class="games-ready-segmented__option" data-games-room-ranked-toggle="false">
            <input
              ...
              aria-label="${escapeHtml(gameT("room.casual"))}"
            >
            <span class="games-ready-segmented__text">${escapeHtml(gameT("room.casual"))}</span>
          </label>
          <label class="games-ready-segmented__option" data-games-room-ranked-toggle="true">
            <input
              ...
              ${room.isRanked ? "checked" : ""}
              ${canManageRanked ? "" : "disabled"}
              aria-label="${escapeHtml(gameT("room.ranked"))}"
            >
            <span class="games-ready-segmented__text">${escapeHtml(gameT("room.ranked"))}</span>
          </label>
        </fieldset>
        ${
          canManageRanked
...
export function getRoomAuthorName(room: GameRoom): string {
  ...
  return author?.name || gameT("room.adminFallback");
}
